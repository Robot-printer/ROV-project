...
from ranger_interfaces.msg import ThrusterThrottle, String
...
class SerialComms(Node):
    ...
    def __init__(self):
        ...
        self.open_serial(comport, baudrate)
        self.serial_debug_publisher = self.create_publisher(String, 'debug_data', 10)
        ...
        self.throttle_subscriber_ = self.create_subscription(ThrusterThrottle, 'throttle_data', self.throttle_data_callback, 10)
        self.timer = self.create_timer(1/sensor_update_frequency, self.timer_callback)
    
    def throttle_data_callback(self, data:ThrusterThrottle):
        message = "MOTOR " + str(data.id) + " " + str(data.value)
        debug_msg = String()
        debug_msg.data = message
        self.serial_debug_publisher.publish(debug_msg)
        self.write_serial(message.encode())
        #time.sleep(0.001)
    ...
    def timer_callback(self):
        ...
        messages = []
        while data == 0:
            data = self.read_serial().decode('utf-8')
            if data != 0:
                messages.append(data)

        for message in messages:
            publishable = self.parse_message(message)
            ...
            if publishable != 0:
                ...
                elif publishable is String:
                    self.serial_debug_publisher.publish(publishable)
    ...
    def parse_message(self, message):
        ...
        match command:
            ...
            case "DEBUG":
                ...
                pub.data = str(message)
                print(message)
                return pub
            case _:
                return 0

    def open_serial(self, comport, baudrate):
        global ser
